...
function AnalysisProcessor({ onAnalysisComplete, duration = 3000 }: AnalysisProcessorProps): JSX.Element {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setProgress((prevProgress) => {
        ...
        const stepIndex = Math.floor((newProgress / 100) * (analysisSteps.length - 1));
        setCurrentStep(analysisSteps[Math.min(stepIndex, analysisSteps.length - 1)]);

        // Update time remaining
        const remainingTime = Math.ceil((duration - (newProgress / 100) * duration) / 1000);
      ...
      <GlassCard padding="lg" className="w-full max-w-2xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-4 gap-2 mt-4">
            {["Aortic", "Pulmonary", "Tricuspid", "Mitral"].map((valve, index) => (
              <div
                key={valve}
